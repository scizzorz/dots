...
export MANPAGER='/bin/bash -c "vim -c \"silent! set ft=man ts=8 cc=0 nomod nolist nu rnu noma\" -c \"noremap q <Esc>:q!<Return>\" -c \"unmap q:\"</dev/tty <(col -b)"'
...
# derp
function vimp {
	/bin/bash -c "vim -c 'silent! set ft=$1 ts=8 cc=0 nomod nolist nu rnu noma' -c 'noremap q <Esc>:q!<Return>' -c 'unmap q:'</dev/tty <(col -b)"
}
