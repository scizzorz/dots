...
export MANPAGER='/bin/bash -c "vim -c \"silent! set ft=man ts=8 cc=0 nomod nolist nonu nornu noma nofen\" -c \"noremap q <Esc>:q!<Return>\" -c \"noremap j <C-e>\" -c \"noremap k <C-y>\"</dev/tty <(col -b)"'
...
# derp
function vimp {
	/bin/bash -c "vim -c 'silent! set ft=$1 ts=8 cc=0 nomod nolist nonu nornu noma nofen' -c 'noremap q <Esc>:q!<Return>' -c 'noremap j <C-e>' -c 'noremap k <C-y>'</dev/tty <(col -b)"
}
